...
5. **배포 PR 생성:** `develop` -> `main` 방향으로 PR을 생성합니다.
    - **타이틀:** `✅ [Deploy] v?.?.? 배포` 패턴 (예: `✅ [Deploy] v1.0.0 배포`)
    - **본문:** 반드시 `Closes #이슈번호` 또는 `Fixes #이슈번호` 키워드를 포함하여 머지 시 이슈가 자동으로 닫히도록 합니다.
    - **버전 규칙:** 
        - `Major (1.0.0)`: 대규모 변경 사항
        - `Minor (0.1.0)`: 새로운 기능 추가
        - `Patch (0.0.1)`: 사소한 버그 수정
6. **PR 머지 코멘트:** 
    - **구조:** 타이틀에 상세 내용을 포함하는 문장을 넣고, 본문(detail)은 비워둡니다. (예: `gh pr merge --merge --subject "✨ feat: 로깅 기능 강화 (#27)" --body ""`)
